...
if ($_GET['condition2'] == '미설치매장') {
  $sql = "SELECT        c.pos_code AS '매장코드', replace(c.CHANNEL,'홈/미디어','스마트홈') AS '영업담당', c.bg_code AS '지원팀', c.mg_code AS '투자유형', c.agency_name AS '운영자명', c.agency_code AS '운영자코드', c.pos_name AS '매장명',
  ...
  where c.pos_code = '".$pos_code."'";}
    ...
    $sql =
    "SELECT
    c.pos_code as '코드', d.pos_id AS '매장코드', replace(c.CHANNEL,'홈/미디어','스마트홈') AS '영업담당', c.bg_code AS '지원팀', c.mg_code AS '투자유형', c.agency_name AS '운영자명', c.agency_code AS '운영자코드', c.pos_name AS '매장명',
    c.pos_address '매장주소', d.time AS '등록일', d.TIME2 AS '접속일'
    FROM
    (
      SELECT UPPER(pos_exec.pos_id)AS `pos_id`, date(min(pos_exec.TIMESTAMP)) AS `time`, date(max(pos_exec.TIMESTAMP)) AS `time2`
...
<body class="body-bg">
  ...
  <div class="horizontal-main-wrapper">
    ...
    <div class="main-content-inner">
      <div class="container">
        <div class="row">
          <div class="col-lg-6 mt-4"><h5>매장정보 상세보기</h5></div>
          <div class="col-lg-6 mt-4" style="text-align: right;"><small>  Main > 매장관리 > 매장정보 상세보기</small></div>
          <html><hr style="margin:1px; "color="black" width=99%></html>
          <!---->
            <div class="card col-lg-12 mt-3">
              <div class="card-body">
                <form action="api/storeReg/modify.php" id="submit_form" onsubmit="return white();" method="post">
                    ...
                    <div class="col-lg-12 mt-3">
                      ...
                      <div class="col-lg-3">
                        <select name="영업담당" style="background-color: #E9ECEF" class="form-control form-control-sm col-sm-7" id="selectID">
                          ...
                          <?
                          $query = "SELECT replace(CHANNEL,'홈/미디어','스마트홈') as 'channel' FROM did_pos_code GROUP BY CHANNEL";
                          $conn->DBQ($query);
                          $conn->DBE(); //쿼리 실행
                      ...
                      <div class="col-lg-3">
                        <select style="background-color: #E9ECEF" name="지원팀" class="form-control form-control-sm col-sm-7" id="good">
                          ...
                          <?
                            $query = "SELECT bg_code FROM did_pos_code where replace(CHANNEL,'홈/미디어','스마트홈') = '".$row['영업담당']."' GROUP BY bg_code";
                            $conn->DBQ($query);
                            $conn->DBE();
                                                                                ...
                                                                                <div class="col-lg-2">
                                                                                  ...
                                                                                  <div class="input-group">
                                                                                    <div class="col-lg-2 container">
                                                                                      <button class="btn btn-lg mr-2 btn btn-xs" type="reset" value=""><a href="javascript:history.go(-1)">취소</a></button>
