...
$db = new DBC;
$db->DBI();
$query= "SELECT * FROM board_list NATURAL JOIN menu_list where idx ='$idx' and b_state <>4";
$db->DBQ($query);
$db->DBE();
$result= $db->DBF();

$note = urldecode($result['note']);
     ...
     if($('input[name=content_source]:checked').val() == 1){
       function modify_video(){
         $.ajax({
           type : 'POST',
           dataType : 'html',
           url : '../sessions/modify_load_video_form.php',
           data :{idx:idx},
           success: function(data){
             $("#content").empty();
             $("#content").append(data);
       ...
       function modify_image(){
         $.ajax({
           type : 'POST',
           dataType : 'html',
           url : '../sessions/modify_load_image_form.php',
           data :{idx:idx},
           success: function(data){
             $("#content").empty();
             $("#content").append(data);
                      ...
                      <form id="validationform" action="../sessions/board_write_ok.php" method="post" data-parsley-validate="" novalidate="">
                        ...
                        ?>
                              ...
                              <div class="col-12 col-sm-8 col-lg-6">
                                <div class="custom-control-inline">
                                  ...
                                  <select class="form-control form-control-sm" name="category" style="margin-right:2vw;width:120px">
                                    <?
                                    while($cate=$db->DBF()){
                                      if($cate['category'] == $result['category']){?>
                                ...
                                <div class="custom-control-inline">
                                  ...
                                  <select class="form-control form-control-sm" name="menu" style="width:200px">
                                    <?
                                    $sql2="SELECT menu_num, menu FROM menu_list where category='{$result['category']}' and state <> 4 ";
                                    $db->DBQ($sql2);
                                    $db->DBE();
                                    while($menu=$db->DBF()){
                                      if($menu['menu'] == $result['menu']){?>
                          ...
                          <div class="form-group row" id="title">
                              ...
                              <div class="col-12 col-sm-8 col-lg-6">
                                  <input type="text" name="title" required="" placeholder="제목" maxlength="15"class="form-control" value="<?=urldecode($result['title'])?>">
                              </div>
                          </div>
                          <div class="form-group row" id="note">
                              <label class="col-12 col-sm-3 col-form-label text-sm-right">내용</label>
                              <div class="col-12 col-sm-8 col-lg-6">
                                  <textarea required="" name="note" class="form-control"><?=$note?></textarea>
                              </div>
                          </div>
                          <div class="form-group row">
                            <label class="col-12 col-sm-3 col-form-label text-sm-right">컨텐츠</label>
                            <? if($result['con_source'] == 2){?>
                                <div class="col-12 col-sm-8 col-lg-6">
                                  <label class="custom-control custom-radio custom-control-inline" id="youtube">
                                      <input type="radio" value="1" name="content_source"  class="custom-control-input"><span class="custom-control-label">유튜브</span>
                                  </label>
                                  <label class="custom-control custom-radio custom-control-inline">
                                      <input type="radio" value="2" name="content_source" checked="" class="custom-control-input"><span class="custom-control-label">이미지</span>
                                  </label>
                                </div>
                            <? }else{?>
                                <div class="col-12 col-sm-8 col-lg-6">
                          ...
                          <div class="form-group row text-right">
                              <div class="col col-sm-10 col-lg-9 offset-sm-1 offset-lg-0">
                                  ...
                                  <a href="board_list.php?mc=게시글관리&sc=게시글%20목록"><button type="button" class="btn btn-space btn-secondary">취소</button></a>   <!--글 상세로 보내거나 onClick="history.back()"-->
                              </div>
                          </div>
                      </form>
                  </div>
