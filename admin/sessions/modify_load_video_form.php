...
$query= "SELECT content1 FROM board_list where idx='{$_POST['idx']}' and b_state<>4";
$db->DBQ($query);
$db->DBE();
$result=$db->DBF();?>
  ...
  <div class="col-12 col-sm-8 col-lg-6">
    ...
    <div class="" style="border:1px solid black; margin-top: 10px">
      <div id="show_video" style="overflow-y:scroll;height:300px">
        <!-- 동영상 목록 -->
        <?
        $query1 = "SELECT con_id,idx FROM contents where idx='{$result['content1']}' and res_type <>4";
        $db->DBQ($query1);
        $db->DBE();
        $info=$db->DBF();
      ...
      <div class='search' style="width:100%; text-align:center; border-top:1px solid gray; padding:10px">
        ...
        <input type="text" name="name" class="form-control" style="display: inline; margin-left:5px; width:60%; background-color:white" readonly value="<?=$info['con_id']?>">
        <input type="hidden" name="idx" readonly value="<?=$info['idx']?>">
      </div>
